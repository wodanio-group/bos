<template>
  ...
  <molecule-dialog-entity-search
    ...
    @select="(id) => {
      updatePersonCompanyRelationItems({ id, role: null, department: null, invoiceRecipient: false });
...
const personCompanyRelationItems = ref<{ id: string, role: string | null, department: string | null, invoiceRecipient: boolean }[]>((props.contact && 'persons' in props.contact)
  ? props.contact.persons
  : (props.contact && 'companies' in props.contact)
    ? props.contact.companies
    : []);
const updatePersonCompanyRelationItems = (value: { id: string, role: string | null, department: string | null, invoiceRecipient: boolean }) => {
