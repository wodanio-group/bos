  ...
  <PageSectionBox class="col-span-12">
      <template #headerLeft>
        <select
          ...
          <option value="all">{{ $t('pes.transferBulk.filter.all') }}</option>
          <option value="pending">{{ $t('pes.transferBulk.filter.pending') }}</option>
          <option value="uploaded">{{ $t('pes.transferBulk.filter.uploaded') }}</option>
        </select>
      </template>
      <template #headerRight>
        <atom-button
          ...
          type="button"
          icon="plus"
          :title="$t('pes.transferBulk.createButton')"
      ...
      <table v-else class="w-full text-sm">
        <thead>
          <tr class="border-b border-secondary-200">
            <th class="px-4 py-2 text-left text-xs font-semibold text-secondary-500">{{ $t('pes.transferBulk.fields.id') }}</th>
            <th class="px-4 py-2 text-left text-xs font-semibold text-secondary-500">{{ $t('pes.transferBulk.fields.createdAt') }}</th>
            <th class="px-4 py-2 text-left text-xs font-semibold text-secondary-500">{{ $t('pes.transferBulk.fields.status') }}</th>
            <th class="px-4 py-2 text-left text-xs font-semibold text-secondary-500">{{ $t('pes.transferBulk.fields.uploadedAt') }}</th>
            ...
            <td class="px-4 py-2">
              <div class="flex items-center justify-end gap-1">
                <atom-button
                  type="button"
                  icon="download"
                  :title="$t('pes.transferBulk.downloadXml')"
                  :outline="true"
                ...
                <atom-button
                  ...
                  type="button"
                  icon="calendar-clock"
                  :title="$t('pes.transferBulk.updateXmlDate')"
                  :outline="true"
                ...
                <atom-button
                  ...
                  type="button"
                  icon="upload"
                  :title="$t('pes.transferBulk.markUploaded')"
                  :outline="true"
    ...
    <SimpleAlertDialog
      ...
      :title="$t('pes.transferBulk.createDialog.title')"
      :description="$t('pes.transferBulk.createDialog.description')"
      :submitButtonTitle="$t('pes.transferBulk.createDialog.submit')"
    ...
    <SimpleAlertDialog
      ...
      :title="$t('pes.transferBulk.markUploaded')"
      :submitButtonTitle="$t('pes.transferBulk.markUploaded')"
...
const allItems = ref<BankTransferBulk[]>([]);
...
const bulkToMarkUploaded = ref<BankTransferBulk | null>(null);
...
const loadItems = async () => {
  loading.value = true;
  try {
    const result = await $fetch<{ items: BankTransferBulk[]; totalItems: number }>('/api/pes/bank-transfer-bulk', {
      query: {
        take,
        page: page.value,
        order: 'DESC',
    ...
    console.error('Failed to load bank transfer bulks', e);
...
const onCreateBulk = async () => {
  try {
    ...
    await loadItems();
    toast.add({ type: 'success', title: $t('pes.transferBulk.toast.createSuccess') });
  } catch {
    toast.add({ type: 'error', title: $t('pes.transferBulk.toast.createError') });
  }
};

const onDownloadXml = async (bulk: BankTransferBulk) => {
  await downloadFile(
...
const onUpdateXmlDate = async (bulk: BankTransferBulk) => {
  try {
    await $fetch(`/api/pes/bank-transfer-bulk/${bulk.id}/xml`, { method: 'PATCH' });
    await loadItems();
    toast.add({ type: 'success', title: $t('pes.transferBulk.toast.updateXmlDateSuccess') });
  } catch {
    toast.add({ type: 'error', title: $t('pes.transferBulk.toast.updateXmlDateError') });
  }
};

const openMarkUploaded = (bulk: BankTransferBulk) => {
...
const onMarkUploaded = async () => {
  ...
  try {
    await $fetch(`/api/pes/bank-transfer-bulk/${bulkToMarkUploaded.value.id}`, {
      method: 'PATCH',
      body: { uploadedAt: new Date().toISOString() },
    });
    bulkToMarkUploaded.value = null;
    await loadItems();
    toast.add({ type: 'success', title: $t('pes.transferBulk.toast.markUploadedSuccess') });
  } catch {
    toast.add({ type: 'error', title: $t('pes.transferBulk.toast.markUploadedError') });
  }
};
